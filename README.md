...
In **AI analysis > Try a scenario**, choose one of the three prepared scenarios and a fictional customer, then click **Run selected demo scenario**. There is no download/upload step required. The app adds a dated interview source and pending hypothesis classification, then places **What changed in this demo** above the overall findings.

The result states a distinct finding, suggested response and next test: H3 tests interpretation of health results; H4 tests affordability rather than more guidance; H5 separates workplace-compliance purchases from concern-driven demand. A before/after table shows pending links and reviewed recommendations. **Review this scenario evidence** opens the exact link for human review. Repeating the same scenario for the same customer reuses the existing link and preserves review decisions. Edited or archived scenario sources are not overwritten. Different scenarios accumulate as evidence; choosing an option without clicking Run does not change data.

The new result is a prepared scenario interpretation, not a strategic decision or a live model inference. A pending interview can change the proposed explanation immediately while leaving the reviewed recommendation unchanged. This difference is shown explicitly. No customer action metrics or decisions are changed. The latest scenario result is retained in project exports and remains a dated run snapshot after later edits.
